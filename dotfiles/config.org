...
* [[https://github.com/dholm/benchmark-init-el][benchmark-init:]] enable benchmarking during intit


To [[https://github.com/dholm/benchmark-init-el?tab=readme-ov-file#usage][show the data:]]
...
#+begin_src elisp
  (use-package gc-maybe
    ...
    ;;(gc-maybe-mode +0) ; set minibuffer hooks and idle timer
    :config
    ;; optionally log GCs in a buffer
    (setopt gc-maybe-log-stats t)
    ;; example function advice
    (setopt gc-maybe-idle-delay 1800)
...
Modern Emacs (28+) can compile Elisp to native machine code using libgccjit for 2-5x performance gains.

Native compilation converts bytecode (.elc) to native machine code (.eln files stored in eln-cache/).
This happens asynchronously in the background on first load, then subsequent startups use the fast native code.

To check if native-comp is working:
- ~M-: (native-comp-available-p)~ should return ~t~
- Check ~*Async-native-compile-log*~ buffer during first startup
- View cache: =~/.emacs.d/eln-cache/=
...
#+begin_src elisp
  (when (and (fboundp 'native-comp-available-p)
    ...
    ;; Silence compiler warnings during async compilation
    ;; These warnings are often about deprecated packages and create noise
    (setq native-comp-async-report-warnings-errors nil)

    ;; Compile loaded .elc files asynchronously in background
    ;; This means first startup compiles, subsequent ones are fast
    (setq native-comp-deferred-compilation t)

    ;; Set compilation parallelism (0 = auto-detect CPU cores)
    ;; Lower this (e.g., 2) if compilation pegs your CPU
    (setq native-comp-async-jobs-number 0)

    ;; Make straight.el use native compilation for installed packages
    (setq straight-native-compile t))

  ;; Helper function to check native-comp status
  (defun dm/native-comp-status ()
    "Show native compilation queue status."
    (interactive)
    (if (native-comp-available-p)
        (let ((queue-length (length (native-comp-async-jobs))))
          (message "Native-comp: %d files in compilation queue. Check *Async-native-compile-log* buffer"
                   queue-length))
      (message "Native compilation not available in this Emacs build")))

  ;; Show startup statistics including native-comp
                  ...
                  (mapatoms
                   (lambda (sym)
                     (when (and (fboundp sym)
                       ...
                       (setq native-count (1+ native-count)))))
                  (message "Emacs started with %d native-compiled functions" native-count)))))
#+end_src
...
#+begin_src elisp
   ...
   (use-package dm-org-agenda :straight (:type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :demand t)
   (use-package dm-pomodoro-nag :straight (:type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t))
   (use-package favurls :straight ( :type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :demand t)
   (use-package dm-mcp-tools :straight ( :type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :after (gptel) :demand t)
   (use-package dm-shutdown :straight ( :type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :after (gptel) :demand t)


  (use-package dm-mcp-tools
     :ensure nil
     :load-path "~/workspace/emacs-nursery/"   ;; or the subdir that contains dm-mcp-tools.el
     :after (:all gptel)
     :demand t)
   ...
   (use-package dm-jira :after embark
     :straight (:type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :demand t)
   (use-package dm-tools
     :straight (:type git :local-repo "~/workspace/emacs-nursery" :no-build t :no-install t) :demand t)

   (use-package quake-frame
     :straight (:type git :host nil :repo "https://codeberg.org/ctietze/quake-frame.el"))


;;  (use-package chronotrack
 ;;;;    :straight (:type git :local-repo "~/workspace/ct-el" :no-build t :no-install t)
   ;;  :after quake-frame
    ;; :demand t)


   (use-package dm-quake-chronotrack
...
use-package debugging and statistics

To see package load times and troubleshoot slow startup:

1. Keep ~use-package-compute-statistics~ enabled (already set below)
2. After Emacs fully loads, run: ~M-x use-package-report~
3. Sort by load time to find slow packages
4. For detailed logging during startup, temporarily set ~use-package-verbose~ to ~t~

#+begin_src emacs-lisp
  (fset 'yes-or-no-p 'y-or-n-p)

  ;; Collect statistics on package load times
  ;; View with: M-x use-package-report
  (setq use-package-compute-statistics t)

  ;; Set to t to see detailed loading messages in *Messages* buffer
  ;; Useful for debugging slow startup - generates lots of output
  (setq use-package-verbose nil)

  (setq use-package-always-ensure t) ;; ensure that packages are installed if not present
#+end_src

Helper function to quickly view package load times:

#+begin_src emacs-lisp
  (defun dm/show-package-load-times ()
    "Open use-package-report to show package load times.
  Packages are sorted by total load time."
    (interactive)
    (if use-package-compute-statistics
        (use-package-report)
      (message "use-package-compute-statistics is not enabled. Set it to t and restart Emacs.")))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Disabled: suspected memory leak via interval/overlay accumulation per buffer
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c d o") #'consult-org-heading)
...
#+begin_src emacs-lisp
    (message "Init org")
    (setq org-archive-location "~/org-roam.archive/%s_archive::")
...
#+begin_src elisp
  (defun dm/theme-balance-checklist-from-file ()
  "Prepare Theme:Balance checklist"
  (with-temp-buffer
    (insert-file-contents "~/org-roam/theme_balance_checklist.org")  ;; Change path as needed
    (buffer-string)))
...
#+begin_src emacs-lisp
                                ...
                                `(
                                 ...
                                 "* %? ")
                                  ...
                                  ("di" "Important thing daily" entry
                                   (file+olp+datetree ,(concat org-directory  "/important_things_daily_2026.org"))
                                   "* Entered on %U
            %?")

                                  ("dj" "Journal" entry
                                   (file+olp+datetree ,(concat org-directory "/personal-daily-2026.org"))
                                   "* Entered on %U
            ...
            %?")
                                              ("W" "TADA. Something good." entry
                                   (file+olp+datetree ,(concat org-directory  "/tada_2026.org"))
                    "* Entered on %U\n%?")

            ("u" "1Unexpected task" entry (function dm/capture-unexpected-task-into-org-roam-daily) "* TODO [u]    %? ")
            ("t" "Test" entry (function (lambda () (set-buffer (org-capture-target-buffer "~/test.org")) (goto-char (point-max)))) "* Level 2" :immediate-finish t)

            ("T" "Theme: Balance" entry (file+olp+datetree ,(concat org-directory  "/theme_balance.org")) "* %U\n%(dm/theme-balance-checklist-from-file)")
            ("r" "Reviews")
            ("rm" "Monthly Review" entry (file+olp+datetree ,(concat org-directory  "/monthly_reviews.org")) "* %U\n%[~/org-roam/monthly-review-template.org]")
            ("rw" "Weekly Review" entry (file+olp+datetree ,(concat org-directory  "/weekly_reviews.org")) "* %U\n%[~/org-roam/weekly-review-template.org]")


            ("U" "URL Bookmark" entry
           (file "~/org-roam/favurls.org")
           "* %^{Headline} %^g
  :PROPERTIES:
  :URL: %^{URL}
  :END:
...
#+begin_src emacs-lisp
    ...
    (setq org-agenda-custom-commands
          ...
          ("NA" "Available NEXT actions"
         ((org-ql-block
           '(and (todo "NEXT")
                 (or (not (scheduled))
                     (scheduled :to today)))
           ((org-ql-block-header "NEXT (available: unscheduled or scheduled <= today)")
            (org-super-agenda-groups '((:auto-map dm/auto-group-by-parent-with-priority
                                        :transformer (dm/add-age-to-item it))))
...
#+begin_src emacs-lisp
  (defun dm/auto-group-by-parent-with-priority (item)
    "Return a grouping key for ITEM that sorts by priority then parent.
  Format: \"N:Parent Name\" where N is priority (1=A, 2=B, 3=C, 4=none)."
    (let* ((marker (or (get-text-property 0 'org-marker item)
                       (get-text-property 0 'org-hd-marker item)))
           ;; Try multiple ways to get priority
           (pri-prop (get-text-property 0 'priority item))
           (pri-face (get-text-property 0 'face item))
           ;; Get priority directly from the org heading
           (pri-from-marker (when marker
                              (org-with-point-at marker
                                (org-get-priority (thing-at-point 'line t)))))
           ;; Also check for [#A] pattern in the item string
           (pri-from-string (when (string-match "\\[#\\([ABC]\\)\\]" item)
                              (pcase (match-string 1 item)
                                ("A" 1) ("B" 2) ("C" 3))))
           (pri-num (or pri-from-string
                        (cond ((and pri-from-marker (>= pri-from-marker 3000)) 1)
                              ((and pri-from-marker (>= pri-from-marker 2000)) 2)
                              ((and pri-from-marker (>= pri-from-marker 1000)) 3)
                              (t 4))))
           (parent (when marker
                     (org-with-point-at marker
                       (if (org-up-heading-safe)
                         ...
                         "No parent")))))
      ;; Return key with priority indicator for sorting
      (let ((pri-label (pcase pri-num (1 "A") (2 "B") (3 "C") (_ "-"))))
        (format "[%s] %s" pri-label (or parent "No parent")))))

  (defun dm/get-state-change-time (target-state)
    "Get the timestamp when the entry transitioned to TARGET-STATE.
Searches for state change notes like: - State \"NEXT\" from \"TODO\" [timestamp]"
    (save-excursion
      (let ((bound (save-excursion (org-end-of-subtree t)))
            (pattern (format "State \"%s\".*?\\[\\([0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\}\\( [A-Za-z]\\{2,3\\}\\)?\\( [0-9]\\{2\\}:[0-9]\\{2\\}\\)?\\)\\]"
                             target-state)))
        (when (re-search-forward pattern bound t)
          (org-time-string-to-time (match-string 1))))))

  (defun dm/add-age-to-item (item)
    "Add age prefix to agenda ITEM string, preserving text properties."
    (if (stringp item)
        (let* ((marker (or (get-text-property 0 'org-marker item)
                           (get-text-property 0 'org-hd-marker item)))
               (age-str "    "))
          (when marker
            (org-with-point-at marker
              (let ((state-change-time (dm/get-state-change-time "NEXT")))
                (when state-change-time
                  (let ((days (- (time-to-days (current-time))
                                 (time-to-days state-change-time))))
                    (setq age-str
                          (cond
                           ((< days 1) "    ")
                           ((< days 7) (format "%3dd" days))
                           ((< days 30) (propertize (format "%3dw" (/ days 7))
                                                    'face 'warning))
                           (t (propertize (format "%3dm" (/ days 30))
                                          'face 'error)))))))))
          ;; Preserve text properties by copying them to the new string
          (let* ((props (text-properties-at 0 item))
                 (new-item (concat age-str " " item)))
            (add-text-properties 0 (length new-item) props new-item)
            new-item))
      ;; If it's a group structure, transform the items within it
      (if (plistp item)
          (let ((items (plist-get item :items)))
            (plist-put item :items (mapcar #'dm/add-age-to-item items))
            item)
        item)))
...
** Magit
#+begin_src elisp
  (straight-use-package 'git-commit-mode)
...
#+begin_src emacs-lisp
  ...
  (message "Done git-auto-commit-mode")
...
1. To add a calendar, go to settings (3 dots) and the find =Calendar ID=
2. If not syncing: do =M-x org-gcal-sync-tokens-clear=. Tokens need to be cleared/refetched when adding a new calendar.
3. Clear the org-id cache too (=rm ~/.emacs.d/.org-id-locations= and =M-x org-id-update-id-location=
...
#+begin_src elisp
  ...
  (use-package org-gcal
    :after org
    :config
    (setq org-gcal-up-days 7)      ;; Days before today
    (setq org-gcal-down-days 30)   ;; Days after today
    (setq org-gcal-auto-archive t)

    (setq org-gcal-remove-api-cancelled-event t) ;; delete removed events without asking.
    ;; find calendar Id in the calendar settings
    (setq org-gcal-header-alist '(("dmitrym@gmail.com" . "#+PROPERTY: TIMELINE_FACE \"pink\"\n"))
          org-gcal-file-alist `(
                                ("dmitrym@gmail.com" .  ,(concat org-directory "gcal.org"))
                           ;;     ("dmitry.markushevich@varsitytutors.com" .  ,(concat org-directory "vt-cal.org"))
                                ("r7d2lpfs96r5l79oltg49rh6rmnjpcs8@import.calendar.google.com" . ,(concat org-directory "kayen-cal.org")) ;; kayen's hockey
                                ("c6oo61f4r9rsboqcisaep2rbsknq87s5@import.calendar.google.com" . ,(concat org-directory "maya-cal.org")) ;; maya ringette
                                ("eok3gdluocm6eg05c4v1t2aq1oiugj8e@import.calendar.google.com" . ,(concat org-directory "diefenbaker-cal.org"))
                                ("e6mpl77k84bv6chkkiqjcdunofsumt7q@import.calendar.google.com" . ,(concat org-directory "mcmath-cal.org"))
...
;;     (setq dm-idle-gcal-sync (run-with-idle-timer (* 60 60 3) t (lambda () (org-gcal-sync))))
;;   (defun dm/set-timer ()
...
#+RESULTS:
: [nil 0 300 0 nil #[nil ((dm/update-agenda-file-list)) nil] nil idle 0 nil]
...
#+begin_src elisp
    (use-package templatel :demand t)
...
* org-dc
#+begin_src elisp
(load-file "~/workspace/org-dynamic-checklists/org-dc.el")
#+end_src
...
(defun dm/ankify-subtree ()
  "Ankify all direct children of the heading at point.
Each child heading will be processed by org-anki-sync-entry."
  (interactive)
  (let* ((heading (cdr (assoc "ITEM" (org-entry-properties))))
         (has-more-siblings t)) ; Flag to track if we have more siblings
    (message "Ankifying children of: \"%s\"" heading)
    (save-excursion
      ...
      (when (org-goto-first-child)
        ...
        (while has-more-siblings
          (org-anki-sync-entry)
          (sleep-for 0.1)
          ;; Try to move to next sibling, exit loop if none left
          (setq has-more-siblings (org-goto-sibling)))))))
#+end_src
...
(org-noter-enable-org-roam-integration)
   ...
   (setq org-noter-max-short-length 0)
...
#+begin_src emacs-lisp
  ;; Disabled - desktop-save-mode is off
...
#+begin_src elisp
    ...
    ;;(require 'vertico-posframe)
      ...
      ;;   :after vertico
      ;;   :straight (:type git :host github :repo "tumashu/vertico-posframe" :branch "main")
...
#+RESULTS:
: #s(hash-table size 65 test eql rehash-size 1.5 rehash-threshold 0.8125 data (:use-package (26408 14712 680892 0) :init (26408 14712 680889 0) :init-secs (0 0 21 0) :use-package-secs (0 0 1804 0)))
...
#+begin_src emacs-lisp
  ;; Disabled - was prompting to save on exit
...
[[https://github.com/Artawower/blamer.el][Blamer]] shows inline git commit messages, like vscode.
...
- =M-x mcp-hub= to see all currently running mcp servers
#+begin_src elisp
    ...
    (setq mcp-hub-servers
          `(
  ;;          ("mcp-eng" . (:command "/Users/dmitrymarkushevich/bin/eng-mcp.sh"))
  ;;          ("atlassian" . (:command "/Users/dmitrymarkushevich/bin/atlassian.sh" :args ())) ;
  ;;          ("context7" . (:command "npx" :args ("-y" "@upstash/context7-mcp@latest")))
  ;;          ("chroma" . (:command "uvx" :args ("chroma-mcp" "--client-type" "persistent" "--data-dir" "/Users/dmitrymarkushevich/chromadb")))
  ;;          ("basic-memory" . (:command "uvx" :args ("basic-memory" "mcp")))
  ;;           ("github" :url "https://api.githubcopilot.com/mcp/" :token ,(lambda () (getenv "GITHUB_TOKEN")))
            ))
    ...
    ;; (setq dot-env-filepath "/Users/dmitrymarkushevich/workspace/eng-mcp-server/.env")
    ;; (dot-env-config)
    ;; (dot-env-get 'JIRA_HOST)
...
#+begin_src elisp
  ...
  (setq org-jira-custom-jqls
      ...
      AND assignee = currentUser()
          ...
          (:jql "parent = MEX-14106 ORDER BY created DESC" :filename "chimera")
          (:jql "assignee = currentUser()" :filename "assigned-to-me")
          (:jql "reporter in (currentUser()) order by created DESC" :filename "created-by-me")
...
Midnight mode cleans up unused buffers. [[https://www.gnu.org/software/emacs/manual/html_mono/emacs.html#Kill-Buffer][manual]].

#+begin_src elisp

  (message "[d] enable midnight mode")
    ;; Midnight mode configuration
  (require 'midnight)

  ;; Automatically clean buffers that haven't been used in the last 3 days
  (setq clean-buffer-list-delay-general (* 3 24 60 60)) ; 3 days in seconds

  ;; Specify buffers to ignore during cleanup
...
From [[https://emacs.stackexchange.com/a/33523/22240][SO]]

#+begin_src elisp
  (message "[d] timestamp message buffer")
  ...
  Activate this advice with:
  (advice-add 'message :before 'sh/ad-timestamp-message)"
    (unless (or (string-equal FORMAT-STRING "%s%s")
                ;; Don't recurse if we're already in *Messages* buffer
                (eq (current-buffer) (get-buffer "*Messages*"))
                ;; Filter out noisy scroll messages
                (string-match-p "^\\(End\\|Beginning\\) of buffer" (or FORMAT-STRING "")))
      (let ((deactivate-mark nil)
            (inhibit-read-only t))
        (with-current-buffer "*Messages*"
          ;; Save point position so we don't jump user's cursor
          (save-excursion
            (goto-char (point-max))
            (unless (bolp)
              (newline))
...
#+begin_src elisp
  (message "Github markdown export")
  (use-package ox-gfm :defer 5 :init (require 'ox-gfm))
...
#+begin_src elisp
  ...
  ;;   :bind (("C-c d a" . aidermacs-transient-menu))
  ;;   :config
  ;;   (setenv "ANTHROPIC_API_KEY" (getenv "CLAUDE_TOKEN"))
  ;;   :custom
...
from here: https://emacsredux.com/blog/2026/04/07/stealing-from-the-best-emacs-configs/
...
#+begin_src elisp
  ...
  (benchmark-init/show-durations-tabulated)
  (with-current-buffer "*Benchmark Init Results Tabulated*"
    (message "%s" (buffer-string)))


  (benchmark-init/show-durations-tree)
    (with-current-buffer "*Benchmark Init Results Tree*"
    (message "%s" (buffer-string)))
