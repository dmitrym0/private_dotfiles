...
# Get the most recent meditation date
LAST_MEDITATION=$(echo "$DATA" | /opt/homebrew/bin/jq -r '.events | sort_by(.startTime) | last | .startTime // empty')

if [ -z "$LAST_MEDITATION" ]; then
    sketchybar --set $NAME icon="🧘‍♂️" label="?"
    exit 0
fi

# Calculate calendar days since last meditation
LAST_DATE=$(/bin/date -j -f "%Y-%m-%dT%H:%M:%S" "${LAST_MEDITATION%%.*}" "+%Y-%m-%d" 2>/dev/null)
