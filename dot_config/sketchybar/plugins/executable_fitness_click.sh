...
# Toggle popup
CURRENT=$(sketchybar --query fitness | /opt/homebrew/bin/jq -r '.popup.drawing')

if [ "$CURRENT" = "on" ]; then
    sketchybar --set fitness popup.drawing=off
    exit 0
fi
...
sketchybar --remove '/fitness.item\..*/' 2>/dev/null
...
for EVENT in $EVENTS; do
    ...
    sketchybar --add item fitness.item.$INDEX popup.fitness \
               --set fitness.item.$INDEX label="$DATE  ${DURATION}min" \
...
sketchybar --set fitness popup.drawing=on
